...
class CartDelivery(MirAction):
    def __init__(
            ...
            action_config):
        MirAction.__init__(
            self, node, name, mir_api, update_handle, fleet_config,
            action_config)

        self.pickup: Pickup = None
        self.dropoff: Dropoff = None
        self.search_timeout = \
            self.action_config.get('search_timeout', 60)  # seconds

        # Store the mission names to be used during the action
        self.dock_to_cart_mission = \
            self.action_config['missions']['dock_to_cart']
        self.pickup_mission = \
            self.action_config['missions']['pickup']
        self.dropoff_mission = \
            self.action_config['missions']['dropoff']
        self.exit_mission = \
            self.action_config['missions']['exit_lot']
        self.footprint_mission = \
            self.action_config['missions'].get('update_footprint')  # Optional

        # Initialize cart marker type
        self.cart_marker_type_guid = \
            self.api.docking_offsets_guid_get(
    ...
    def perform_action(self, category, description, execution):
        ...
        match category:
            case 'delivery_pickup':
                ...
                if self.is_latch_open():
                    ...
                    self.node.get_logger().info(
                        f'Robot [{self.name}] latch is open, unable to '
                        f'perform pickup, cancelling task...')
                    self.cancel_task()
                    return

                self.node.get_logger().info(
                    f'New pickup requested for robot [{self.name}]')
                self.pickup = Pickup(
            ...
            case 'delivery_dropoff':
                self.node.get_logger().info(
                    f'[{self.name}] Received dropoff request!')
            ...
            case _:
                self.node.get_logger().info(
                    f'Invalid perform action [{category}] passed to '
                    f'CartDelivery, ending action.')
                execution.finished()

    def update_action(self):
        # There should not be a pickup and dropoff being performed
        # simultaneously at any given time, since actions are dispatched
        # sequentially
        if self.pickup:
        ...
        match pickup.state:
            case PickupState.AT_PICKUP:
                ...
                if not mir_pos:
                    self.node.get_logger().info(
                        ...
                        f'[{self.name}], cancelling task')
                    self.cancel_task()
                    pickup.state = PickupState.TASK_CANCELLED
                    return
                cart_marker_guid = mir_pos['guid']
                cart_marker_param = self.api.get_mission_params_with_value(
                    self.dock_to_cart_mission,
                    'docking',
                    'cart_marker',
                    cart_marker_guid)
                marker_type_param = self.api.get_mission_params_with_value(
                    self.dock_to_cart_mission,
                    'docking',
                    'cart_marker_type',
                    self.cart_marker_type_guid)
                ...
                self.node.get_logger().info(
                    f'[{self.name}] Dock to cart mission requested with '
            ...
            case PickupState.DOCK_REQUESTED:
                ...
                if not pickup.mission:
                    self.node.get_logger().info(
                        f'Robot [{self.name}] is indicated to be at the '
                ...
                if self.api.mission_completed(pickup.mission.queue_id):
                    self.node.get_logger().info(
                        f'Robot [{self.name}] dock to cart mission '
            ...
            case PickupState.DOCK_COMPLETED:
                ...
                if cart_check:
                    ...
                    self.node.get_logger().info(
                        f'Robot [{self.name}] found the correct cart, pickup '
                    ...
                    self.node.get_logger().info(
                        f'Robot [{self.name}] was unable to dock under any '
                    ...
                    # exit from the lot
                    self.node.get_logger().info(
                        f'Robot [{self.name}] found the wrong cart, exiting '
            ...
            case PickupState.PICKUP_SUCCESS:
                ...
                self.node.get_logger().info(
                    f'Robot [{self.name}] successfully received cart, '
            ...
            case PickupState.TASK_CANCELLED:
                self.node.get_logger().info(
                    f'Robot [{self.name}] is in pickup cancelled state.')
                # If some MiR mission is in progress, we abort it unless
                # it is latching
                if pickup.mission and not self.api.mission_completed(
                        pickup.mission.queue_id):
                    if pickup.latching:
                        self.node.get_logger().info(
                            f'Robot [{self.name}] is performing latching, '
    ...
    def update_dropoff(self, dropoff: Dropoff):
        ...
        if dropoff.execution is not None and not dropoff.execution.okay():
            ...
            if dropoff.mission and not self.api.mission_completed(
        ...
        if dropoff.mission is None:
            ...
            dropoff.mission = Mission(mission_queue_id,
                                      now)
            self.node.get_logger().info(
                f'[{self.name}] Dropoff mission requested with mission queue '
        ...
        # Mission queued, check completion
            ...
            if self.api.mission_completed(dropoff.mission.queue_id):
                self.node.get_logger().info(
                    f'[{self.name}] Dropoff mission completed!')
                ...
                self.node.get_logger().info(
                    f'[{self.name}] Dropoff mission in progress...')
        return
    ...
    def release_cart(self):
        ...
        if self.is_latch_open():
            self.node.get_logger().info(
                f'Robot [{self.name}] detected to have latch open, dropping '
                f'off cart...')
            self.dropoff = Dropoff(
                execution=None,
                mission_queue_id=None)
            # Dropoff mission will take care of the lot exit,
            # so we can return here
            return
        # If robot is under a cart, exit lot
        if self.is_under_cart():
            self.node.get_logger().info(
                f'Cart detected above robot [{self.name}] during a release '
                f'call, exiting lot...')
            self.exit_lot()

    # Optional function for updating robot footprint by providing
    # footprint guid
    def update_footprint(self, ft_guid: str):
        if not self.footprint_mission:
            return
        ft_params = self.api.get_mission_params_with_value(
            self.footprint_mission, 'set_footprint', 'footprint', ft_guid)
        return self.api.queue_mission_by_name(
            self.footprint_mission, ft_params)
    ...
    def retrieve_mir_coordinates(self, waypoint_name: str):
        ...
        rmf_pose = \
            self.fleet_config.graph.find_waypoint(waypoint_name).location
